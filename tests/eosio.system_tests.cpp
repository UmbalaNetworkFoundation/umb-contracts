...
BOOST_AUTO_TEST_SUITE(eosio_system_tests) // @suppress("Type cannot be resolved")
...
try
{
   ...
   wdump(("Voter info 1: ")(get_voter_info("alice1111111")));
   ...
   wdump(("Voter info 2: ")(get_voter_info("alice1111111")));
   BOOST_REQUIRE_EQUAL(success(), unstake("alice1111111", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000")));
   BOOST_REQUIRE_EQUAL(core_from_string("700.0000"), get_balance("alice1111111"));
   wdump(("Voter info 3: ")(get_voter_info("alice1111111")));
   produce_block(fc::hours(3 * 24 - 1));
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL(core_from_string("700.0000"), get_balance("alice1111111"));
   wdump(("Voter info 4: ")(get_voter_info("alice1111111")));
...
try
{
   ...
   BOOST_REQUIRE_EQUAL(error("missing authority of alice1111111"),
                       push_action(N(alice1111111), N(undelegatebw), mvo()("from", "alice1111111")("receiver", "bob111111111")("unstake_net_quantity", core_from_string("200.0000"))("unstake_cpu_quantity", core_from_string("100.0000"))("transfer", 0), false));
...
BOOST_FIXTURE_TEST_CASE(more_than_50_producer_voting, eosio_system_tester)
try
{
   issue("bob111111111", core_from_string("2000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("bob111111111", core_from_string("13.0000"), core_from_string("0.5791")));
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("attempt to vote for too many producers"),
                       vote(N(bob111111111), vector<account_name>(51, N(alice1111111))));
}
FC_LOG_AND_RETHROW()
...
try
{
   ...
   const std::vector<account_name> voters = {N(producvotera), N(producvoterb), N(producvoterc), N(producvoterd), N(producvotere), N(producvoterf)};
   ...
   // producvoterd votes for abcproducera ... abcproducern
   {
      ...
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterb), vector<account_name>(producer_names.begin(), producer_names.begin() + 41)));
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterc), vector<account_name>(producer_names.begin(), producer_names.begin() + 50)));
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterd), vector<account_name>(producer_names.begin() + 50, producer_names.begin() + 100)));
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvotere), vector<account_name>(producer_names.begin() + 100, producer_names.begin() + 108)));
   }

   {
      auto prod1 = get_producer_info(Ns(producer_names[0]));
      auto prod10 = get_producer_info(Ns(producer_names[9]));
      auto prod11 = get_producer_info(Ns(producer_names[10]));
      auto prod41 = get_producer_info(Ns(producer_names[40]));
      auto prod42 = get_producer_info(Ns(producer_names[41]));
      auto prod50 = get_producer_info(Ns(producer_names[49]));
      auto prod108 = get_producer_info(Ns(producer_names[107]));

      BOOST_REQUIRE(0 == prod1["unpaid_blocks"].as<uint32_t>() && 0 == prod108["unpaid_blocks"].as<uint32_t>());
      BOOST_REQUIRE(0 == prod1["last_claim_time"].as<uint64_t>() && 0 == prod108["last_claim_time"].as<uint64_t>());

      // check vote ratios
      BOOST_REQUIRE(0 < prod1["total_votes"].as<double>() && 0 < prod1["total_votes"].as<double>());
      BOOST_TEST(prod1["total_votes"].as<double>() == prod10["total_votes"].as<double>());
      BOOST_TEST(prod11["total_votes"].as<double>() == prod41["total_votes"].as<double>());
      BOOST_TEST(prod42["total_votes"].as<double>() == prod50["total_votes"].as<double>());
      BOOST_TEST(prod42["total_votes"].as<double>() == prod108["total_votes"].as<double>());
      BOOST_TEST(2 * prod1["total_votes"].as<double>() == 3 * prod11["total_votes"].as<double>());
      BOOST_TEST(prod1["total_votes"].as<double>() == 3 * prod42["total_votes"].as<double>());
   }
   // give a chance for everyone to produce blocks
   {
      produce_blocks(45 * 8 + 45);
      auto active_bps = control->active_producers().producers;
      for (auto v = active_bps.begin(); v != active_bps.end(); ++v)
      {
         cout << "BP[i]" << v->producer_name << endl;
      }
      bool all_41_produced = true;
      for (uint32_t i = 0; i < 41; ++i)
      {
         auto info = get_producer_info(producer_names[i]);
         cout << "Producer " << producer_names[i] << " => "
              << "vote: " << info["total_votes"] << ", un_paided: " << info["unpaid_blocks"].as<uint32_t>() << endl;
         if (0 == get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
         {
            all_41_produced = false;
         }
      }
      bool out_top45_produced = false;
      for (uint32_t i = 45; i < producer_names.size(); ++i)
      {
         if (0 < get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
         {
            out_top45_produced = true;
         }
      }
      BOOST_REQUIRE(all_41_produced && out_top45_produced);
   }
...
try
{
   ...
   {
      // cout << " Request permission from " << name{x} << endl;
   ...
   // one more approval
   BOOST_REQUIRE_EQUAL(success(), push_action_msig(name(producer_names[30]), N(approve), mvo()("proposer", "alice1111111")("proposal_name", "upgrade1")("level", permission_level{name(producer_names[30]), config::active_name})));
...
try
{
   ...
   create_account_with_resources(N(producvoterd), config::system_account_name, core_from_string("1.0000"), false, large_asset, large_asset);
   ...
   BOOST_CHECK_EQUAL(wasm_assert_msg("cannot undelegate bandwidth until the chain is activated (at least 15% of all tokens participate in voting)"),
                     unstake("producvotera", core_from_string("50.0000"), core_from_string("0.0000")));
   ...
   // give a chance for everyone to produce blocks
   {
      ...
      bool rest_didnt_produce = true;
      for (uint32_t i = 41; i < producer_names.size(); ++i)
      {
         if (0 < get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
         {
            rest_didnt_produce = false;
      ...
      BOOST_REQUIRE_EQUAL(false, all_41_produced);
   ...
   transfer(config::system_account_name, "producvoterd", core_from_string("100000000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvoterd", core_from_string("2000000.0000"), core_from_string("2000000.0000")));

   BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterb), vector<account_name>(producer_names.begin(), producer_names.begin() + 41)));
   BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterc), vector<account_name>(producer_names.begin() + 41, producer_names.begin() + 91)));
   BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterd), vector<account_name>(producer_names.begin() + 91, producer_names.begin() + 108)));

   // give a chance for everyone to produce blocks
   {
      produce_blocks(45 * 12);
      bool all_41_produced = true;
      for (uint32_t i = 0; i < 41; ++i)
      {
         if (0 == get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
      ...
      bool top_108_can_produce = false;
      for (uint32_t i = 45; i < producer_names.size(); ++i)
      {
         if (0 < get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
         {
            top_108_can_produce = true;
         }
      }
      BOOST_REQUIRE_EQUAL(true, all_41_produced);
      BOOST_REQUIRE_EQUAL(true, top_108_can_produce);
      BOOST_REQUIRE_EQUAL(success(),
                          push_action(producer_names.front(), N(claimrewards), mvo()("owner", producer_names.front())));
...
try
{
   ...
   for (const auto &v : voters)
   {
      ...
      if (v == N(producvoterc))
      {
         // voter c will have less stake
         BOOST_REQUIRE_EQUAL(success(), stake(v, core_from_string("20000000.0000"), core_from_string("20000000.0000")));
      }
      else
         BOOST_REQUIRE_EQUAL(success(), stake(v, core_from_string("30000000.0000"), core_from_string("30000000.0000")));
   }
   ...
   {
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvotera), vector<account_name>(producer_names.begin(), producer_names.begin() + 50)));
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterb), vector<account_name>(producer_names.begin() + 50, producer_names.begin() + 100)));
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterc), vector<account_name>(producer_names.begin() + 100, producer_names.begin() + 108)));
   }

   // give a chance for everyone to produce blocks
   {
      produce_blocks(50 * 6 + 44);
      bool all_top_41_produced = true;
      for (uint32_t i = 0; i < 41; ++i)
      {
         if (0 == get_producer_info(producer_names[i])["unpaid_blocks"].as<uint32_t>())
         {
            all_top_41_produced = false;
         }
      }
      bool top_108_can_produce = false;
      for (uint32_t i = 45; i < producer_names.size(); ++i)
      ...
      BOOST_REQUIRE(all_top_41_produced && top_108_can_produce);
   }

   {
      //TODO: better design test to prevent randomization
      produce_block(fc::hours(7));
      const uint32_t voted_out_index = 100;
      const uint32_t new_prod_index = 108;
      ...
      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterd), vector<account_name>(producer_names.begin() + 101, producer_names.begin() + 109)));
      // voter will have less
      BOOST_REQUIRE_EQUAL(0, get_producer_info(producer_names[new_prod_index])["unpaid_blocks"].as<uint32_t>());
      produce_blocks(4 * 12 * 45);
...
try
{
   ...
   BOOST_REQUIRE_EQUAL(success(), updaterscore("alice"));
   BOOST_REQUIRE_EQUAL(core_from_string("1200.0000"), get_pending_powerscore("alice"));
   produce_block(fc::hours(24));
   produce_blocks(2);
