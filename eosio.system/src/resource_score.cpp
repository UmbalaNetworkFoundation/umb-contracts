...
inline float  calculation_resource_usage_weight(float ratio, float usage) 
{
...
void system_contract::update_resource_score(const name account)
{
    ...
    */
    ...
    float net_score = calculation_resource_usage_weight(net_usage_ratio, net_weight) * net_weight;
    print("Net Score: ", net_score);

    float   cpu_usage_ratio = (float) (cpu_weight/cpu_weight_limit);
    float cpu_score =  calculation_resource_usage_weight(cpu_usage_ratio, cpu_weight) * cpu_weight;
